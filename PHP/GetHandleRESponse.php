...
try
{
	...
	if ($strResponseIPAdd != "221.134.101.174" && $strResponseIPAdd != "221.134.101.169" && $strResponseIPAdd != "198.64.129.10" && $strResponseIPAdd != "198.64.133.213")
	{
		...
		*/
		$REDIRECT = 'REDIRECT=http://flyhi.in/PHP/StatusTRAN.php?ResError=--IP MISSMATCH-- Response IP Address is: '.$strResponseIPAdd.'&chk=1';
		echo $REDIRECT;
	}
	else
	{	
...
*/
		...
		{          
					...
					*/
					$strHashTraportalID=trim("9000525"); //USE Tranportal ID FIELD FOR HASHING ,Mercahnt need to take this filed value  from his Secure channel such as DATABASE.
					...
					/* Hashing Response Successful	*/
						...
						$REDIRECT = 'REDIRECT=http://flyhi.in/PHP/StatusTRAN.php?ResResult='.$ResResult.'&ResTrackId='.$ResTrackID.'&ResAmount='.$ResAmount.'&ResPaymentId='.$ResPaymentId.'&ResRef='.$ResRef.'&ResTranId='.$ResTranId.'&ResError='.$ResErrorText.'Hashing Response Successful&chk=2';
						echo $REDIRECT;
					}
					else
					{
					...
					$REDIRECT = 'REDIRECT=http://flyhi.in/PHP/StatusTRAN.php?ResError=Hashing Response Mismatch&chk=3';
					echo $REDIRECT;														
					}
		}
		else 
		{
						...
						TRANACTION PAYMENT STATUS IN MERCHANT DATABASE AT THIS POSITION 
						AND THEN REDIRECT CUSTOMER ON RESULT PAGE*/
		$REDIRECT = 'REDIRECT=http://flyhi.in/PHP/StatusTRAN.php?ResResult='.$ResResult.'&ResTrackId='.$ResTrackID.'&ResAmount='.$ResAmount.'&ResPaymentId='.$ResPaymentId.'&ResRef='.$ResRef.'&ResTranId='.$ResTranId.'&ResError='.$ResErrorText.'&chk=4';		
		echo $REDIRECT;
		}
	}	
}
